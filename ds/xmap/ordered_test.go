...
func TestOrdered(t *testing.T) {
	check := func(t *testing.T, m1 testOrderedType[int, string]) {
		...
		xt.Equal(t, m1.Map(false), map[int]string{0: "v0", 1: "v1"})
		xt.Equal(t, m1.Map(true), map[int]string{0: "v0", 1: "v1"})
		...
		for i := 2; i < 10; i++ {
			...
			m1.Set(i, val)
			xt.Equal(t, m1.MustGet(i), val)
		}
		xt.Equal(t, m1.Keys(), keys)
		xt.Equal(t, m1.Len(), 10)
		var keys1 []int
		m1.Range(func(key int, value string) bool {
			keys1 = append(keys1, key)
			return true
		})
		xt.Equal(t, keys1, keys)

		m1.Delete(1, 3)
		keys2 := []int{0, 2, 4, 5, 6, 7, 8, 9}
		xt.Equal(t, m1.Keys(), keys2)

		xt.Equal(t, m1.GetDf(2, ""), "v_2")
		xt.Equal(t, m1.GetDf(0, ""), "v0")
		m1.Clear()
		xt.Equal(t, m1.Len(), 0)
		xt.Empty(t, m1.Keys())
