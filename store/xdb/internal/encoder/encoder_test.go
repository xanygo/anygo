...
func TestEncoder_Fields(t *testing.T) {
	dz := dialect.Postgres{}
	t.Run("case 1", func(t *testing.T) {
		var u testUser5
		enc := &encoder.Encoder[testUser5]{
			Dialect: dz,
		}
		got, err := enc.Fields(u)
		xt.NoError(t, err)
		xt.Equal(t, got, []string{"csv1", "mp1"})
	})
	...
	t.Run("case 2", func(t *testing.T) {
		...
		got, err := enc.Fields(u) // nil ptr
		xt.NoError(t, err)
		xt.Equal(t, got, []string{"csv1", "mp1"})

		got, err = enc.Fields(&testUser5{})
		xt.NoError(t, err)
		xt.Equal(t, got, []string{"csv1", "mp1"})
	})
	...
	t.Run("case 3", func(t *testing.T) {
		...
		enc := &encoder.Encoder[map[string]string]{
			Dialect: dz,
		}
		got, err := enc.Fields(u) // nil ptr
		xt.NoError(t, err)
