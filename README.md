...
Storage：
- xredis: Redis Client 使用 resp3 协议的 Redis Client
- xdb: DataBase Client, 内置拦截器功能，轻量 ORM 支持，内置已支持 MariaDB、MySQL、SQL Server、Sqlite、Postgres
  - 注：需要自己注册对应数据库的驱动 
- xcache: 支持泛型的缓存组件，已内置支持多种存储引擎。
...
- xkv：通用的，仿 Redis 存储的 Key-Value API，已支持数据类型 String、List、Hash、Set，ZSet。已内置支持驱动：
  1. File Storage：本地文件系统存储
...
- xlog：基于标准库 slog 封装的日志库，轻量好用
- xt：支持泛型的轻量的单侧 assert 库 
- xi18n：国际化（i18n）的能力
- 一些常用算法实现
